...
A RAG system inspired by [globalwork.ai](https://globalwork.ai), built around mock IT vacancy data from `vacancies.json` at the project root, with optional CV upload (PDF) for personalized matching. The app is a single-window, single-user chat — the emphasis is on RAG-core, not on UX.
